...
type WidgetTextProps = { item?: any; onChange?: any; onRemove?: any };

function WidgetText({ item, onChange, onRemove }: WidgetTextProps) {
    ...
    <div className="mb-2 border-1 hover:border-primary border-dashed relative flex justify-between items-center">
      {isEdit ? (
        ...
        <div className="flex-1" onClick={() => setIsEdit(true)}>
          <p className={`${item.value ? "text-black" : "text-default-400"}`}>
